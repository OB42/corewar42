...
/*   Updated: 2018/03/08 17:08:39 by vburidar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_ldi(t_ins *ins, t_proc *proc)
{
	...
	if ((ins->ocp & 40) && (ins->ocp & 80))
		val_1 = *(ft_oob(proc->init, proc->curseur + ins->param[0] % IDX_MOD));
	else if(ins->ocp & 40)
		val_1 = proc->reg[ins->param[0]];
	else if (ins->ocp & 80)
		val_1 = ins->param[0];
	if ((ins->ocp & 10) && (ins->ocp & 20))
		val_2 = *(ft_oob(proc->init, proc->curseur + ins->param[1] % IDX_MOD));
	else if (ins->ocp & 20)
		val_2 = ins->param[1];
